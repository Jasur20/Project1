...
func preCheck(ctx *gin.Context) {
	...
	if bankcardtype != "BRT card" {
		...
		var responseNPS models.ResponceNPC 

		resp, err := client.Post("http://localhost/getCardInfoKM?pan=" + PAN + "?exp=" + ExpDate + "?agentlogin=" + AgentLogin + "?brand=" + Brand,"application/x-www-form-urlencoded",nil)
		...
		err=xml.Unmarshal([]byte(resString),&responseNPS)
		if err != nil {
			errResp.Status = 404
			errResp.StatusDetails = err.Error()
			ctx.JSON(http.StatusBadRequest, errResp)
		...
		ctx.JSON(http.StatusOK,"the card exists")
	}

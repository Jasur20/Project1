...
type endPointController struct {
	engine      *gin.Engine
	integration usecase.Integration
	token       string
}
...
func NewEndpointController(token string, engine *gin.Engine, integration usecase.Integration) EndPointController {
...
func (e *endPointController) Payment(ctx *gin.Context) {
	...
	var paymentReq usecase.PaymentRequestBody
...
func (e *endPointController) ReceiverInfo(ctx *gin.Context) {
	...
	var req usecase.GetReceiverInfoRequestBody
	body, err := io.ReadAll(ctx.Request.Body)
	if err != nil {
		logrus.WithError(err).Warn("on request reading")
		ctx.JSON(http.StatusBadRequest, "Bad request")
...
func (e *endPointController) PaymentStatus(c *gin.Context) {
	...
	var req usecase.PostCheckRequestBody
